...
permalink: /getting-started/windows-texlive/vscode/
prev_tutorial: /getting-started/windows-texlive/ktug-repo/
next_tutorial: /getting-started/windows-texlive/lshort/
---
...
<img src="{{ '/assets/images/windows-texlive/vscode-inst.png' | relative_url }}" alt="VS Code 설치">

그러면 VS Code의 설치가 시작됩니다.

<img src="{{ '/assets/images/windows-texlive/vscode-inst-gui.png' | relative_url }}" alt="VS Code 설치 진행">

설치가 완료된 후에 `Windows`를 눌러서 `code`라고 입력하여 이 앱을 잘 실행할 수 있는지 확인합니다.

<img src="{{ '/assets/images/windows-texlive/win-menu-vscode.png' | relative_url }}" alt="VS Code 실행">
...
<img src="{{ '/assets/images/windows-texlive/vscode-init.png' | relative_url }}" alt="VS Code 첫 실행">
...
<img src="{{ '/assets/images/windows-texlive/vscode-market.png' | relative_url }}" alt="마켓플레이스">

제일 위에 있는 검색 바에 "korean"이라고 써넣으면 한국어 언어 패키지를 설치할 수 있습니다. 다음 그림과 같이 진행됩니다. 언어 패키지가 활성화된 이후에 VS Code를 한 번 닫았다가 다시 실행하여야 합니다.

<img src="{{ '/assets/images/windows-texlive/vscode-korean.png' | relative_url }}" alt="한국어 언어 팩 설치">
...
먼저, 작업 폴더를 하나 만듭니다. `탐색기`에서 홈(예시에서는 `KTUG`) 폴더 아래에 `ktug-latex-exe`이라는 폴더를 작성합니다. (우클릭 후에 "새로 만들기"에서 "폴더"를 선택하여 새 폴더를 만들 수 있습니다.)

<img src="{{ '/assets/images/windows-texlive/expl-new-dir.png' | relative_url }}" alt="새 폴더 만들기">

> **주의** Windows용 TeX Live에서는 파일과 폴더 이름에 한글이나 띄어쓰기가 포함되지 않아야 하기에 파일과 폴더 이름을 ASCII 문자로만 띄어쓰기 없이 짓도록 권장합니다. 자유로운 파일과 폴더 이름 짓기가 중요하다면 MiKTeX을 사용해야 합니다.

이제 VS Code에서 **탐색기 버튼**(왼쪽 바의 제일 위에 있는 아이콘)을 누르고 "폴더 열기"를 선택하여 방금 만든 폴더를 엽니다.

<img src="{{ '/assets/images/windows-texlive/vscode-open-folder.png' | relative_url }}" alt="폴더 열기">
...
<img src="{{ '/assets/images/windows-texlive/vscode-trust-folder.png' | relative_url }}" alt="신뢰할 수 있는 작업 공간">
...
<img src="{{ '/assets/images/windows-texlive/vscode-pref.png' | relative_url }}" alt="사용자 설정">

설정에서는 폰트 크기와 폰트 종류 정도를 지정해봅니다. 다음 그림은 현재 필자의 설정입니다.

<img src="{{ '/assets/images/windows-texlive/vscode-gen-settings.png' | relative_url }}" alt="폰트 설정">
...
<img src="{{ '/assets/images/windows-texlive/vscode-latexworkshop.png' | relative_url }}" alt="LaTeX Workshop 메뉴">

> **참고** 현재까지 진행한 상태에서 tex 파일을 저장하면 latexworkshop의 기본 설정에 따라서 latexmk/pdflatex이 동작하면서 build를 시도합니다. 그러나 만약 소스가 XeLaTeX을 위한 것이라면 폰트 활용 관련 이런저런 에러가 발생했다면서 귀찮은 메시지가 나타날 것입니다. 아래 이어지는 설정을 적용하면 이 문제는 사라지므로 일단 무시합니다.
...
<img src="{{ '/assets/images/windows-texlive/vscode-user-settings-json.png' | relative_url }}" alt="settings.json">

이제 `Ctrl+Alt+B`를 눌러서 현재 편집 중인 파일을 컴파일해봅니다. 컴파일 과정을 보려면 `Ctrl+Shift+U`를 누른 다음 "작업"을 "LaTeX Compiler"로 하면 됩니다.

<img src="{{ '/assets/images/windows-texlive/vscode-compiler.png' | relative_url }}" alt="컴파일 진행">
...
> **참고** 이 안내서 작성자의 개인적인 취향 때문에 처음에는 외장 뷰어를 설치하고 설정하는 내용을 포함하고 있었습니다. 그러나 그 과정이 너무 번거로워서 처음 사용자를 당황하게 만든다는 지적이 있어서 이를 부록으로 돌렸습니다. 관심있는 분은 <a href="{{ '/getting-started/windows-texlive/utils/' | relative_url }}">부록</a>의 SumatraPDF 관련 내용을 참고하기 바랍니다.

## 다음 단계

Visual Studio Code 설정이 완료되었습니다. 이제 <a href="{{ '/getting-started/windows-texlive/lshort/' | relative_url }}">lshort-ko 학습하기</a>로 진행하세요.
