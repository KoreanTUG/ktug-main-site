...
permalink: /getting-started/windows/miktex/utils/
prev_tutorial: /getting-started/windows/miktex/lshort/
next_tutorial: /getting-started/windows/miktex/ktug-repo-adv/
---

## Python과 latexminted

MiKTeX에서 [Python](https://www.python.org)이 필요한 이유는 주로 `latexminted`라는 유틸리티 때문입니다. `minted` 패키지로 소스코드리스팅을 하는 경우에 이 유틸리티의 도움이 필요합니다. (참고로, `latexminted`를 설치하면 `pygments` 패키지도 함께 설치해줍니다. TeX Live와 달리 MiKTeX은 이 유틸리티들을 별도로 설치하여야 합니다.)
...
설치 후에 `터미널`을 한 번 닫았다가 열어줍니다. 그리고 다음 명령을 다시 내립니다.

```powershell
pip install latexminted
```

> **참고** 만약 이 단계에서 `pip`라는 명령이 없다고 하면서 실행되지 않으면 Python이 설치된 폴더를 `PATH` 환경 변수에 추가하여야 하는데, 이 문제는 일반적으로 `winget`으로 설치한 경우에 발생하지 않을 것입니다.

> **참고** `minted` 패키지가 사용된 문서는 `-shell-escape`라는 옵션을 붙여서 컴파일하여야 합니다.

> **참고** pip로 설치한 스크립트의 실행 위치가 Path에 포함되어 있지 않다면 이를 수동으로 등록해주어야 합니다. 예를 들면 `C:\Users\<username>\AppData\Roaming\Python\Python313\Scripts`와 같은 주소를 사용자 PATH에 추가하여야 합니다.

## VS Code의 외부 뷰어

VS Code의 PDF Viewer를 (내장 뷰어가 아니라) [SumatraPDF](https://www.sumatrapdfreader.org)라는 외부 유틸리티로 설정하려면 다음과 같이 합니다.
...
그리고 `C:/Users/sungguri` 부분을 자신의 "이름"으로 바꾸어 넣습니다. 예컨대 이름이 `chunhyang`이라면
...
"latex-workshop.view.pdf.external.synctex.command": "C:/Users/sungguri/AppData/Local/SumatraPDF/SumatraPDF.exe",
...
"latex-workshop.view.pdf.external.viewer.command": "C:/Users/sungguri/AppData/Local/SumatraPDF/SumatraPDF.exe",
```
...
기타 유틸리티 설치가 완료되었습니다. 필요에 따라 <a href="{{ '/getting-started/windows/miktex/ktug-repo-adv/' | relative_url }}">KTUG 사설저장소 추가 설정</a>을 참고하세요.
