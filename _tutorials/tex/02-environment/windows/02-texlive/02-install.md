...
permalink: /tutorials/tex/windows/texlive/install/
prev_tutorial: /tutorials/tex/windows/texlive/prep/
next_tutorial: /tutorials/tex/windows/texlive/texworks/
---
...
![Winget으로 cURL 탐색 및 설치](/assets/images/windows/texlive/winget-inst-curl.png)
...
![cURL로 TeX Live ISO 이미지 내려받기](/assets/images/windows/texlive/curl-tl-iso-down.png)
...
![TeX Live ISO 이미지 탑재](/assets/images/windows/texlive/expl-iso-mount.png)
...
![TeX Live 설치 프로그램 실행](/assets/images/windows/texlive/expl-iso-mounted.png)

파일 확장자가 보이지 않으면 아래와 **보기** > **표시** > **파일 확장명**을 선택하여 파일 확장명를 표시하게 하면 편리합니다.

![탐색기로 확장자 보기](/assets/images/windows/texlive/expl-show-ext.png)

`install-tl-windows.bat`을 실행하면 다음과 같이 첫 화면이 표시됩니다.

![TeX Live 설치 프로그램 첫 화면](/assets/images/windows/texlive/tl-inst-init.png)

위 화면이 표시되면서 터미널 창도 함께 표시되는데 닫지 말고 그대로 둡니다. 이어서 다음 창이 표시되면 **Install** 버튼을 눌러 설치를 시작합니다.

![TeX Live 설치 프로그램 시작 화면](/assets/images/windows/texlive/tl-inst-start.png)

설치가 시작되면 아래와 같이 로그 메시지가 화면에 표시되며 설치가 진행됩니다. TeX Live에는 크기가 작은 파일들의 개수가 많아 설치에 긴 시간이 걸립니다.

![TeX Live 설치 중 화면](/assets/images/windows/texlive/tl-inst-middle.png)
...
![TeX Live 설치 완료 화면](/assets/images/windows/texlive/tl-inst-done.png)
...
![TLShell 첫 화면](/assets/images/windows/texlive/tlshell-init.png)
...
![TLShell 미러 저장소 선택](/assets/images/windows/texlive/tlshell-repo-sel-nb.png)

그러면 다음과 같이 새로 지정한 CTAN 미러가 표시됩니다. 이제 **Save and Load** 버튼을 누릅니다.

![TLShell 미러 저장소 선택 완료](/assets/images/windows/texlive/tlshell-repo-set.png)

이제 새로 지정한 CTAN 미러로부터 패키지 정보를 읽어들여 아래와 같이 표시면 **Close** 버튼을 누릅니다.

![TLShell 미러 저장소 로드](/assets/images/windows/texlive/tlshell-repo-load.png)
...
![TLShell 주화면](/assets/images/windows/texlive/tlshell-after-load.png)

그러면 다음과 같이 `cmd` 창이 열리면서 `tlmgr`의 업데이트가 진행됩니다. 업데이트가 끝나면 이 `cmd` 창을 닫습니다.

![tlmgr 업데이트](/assets/images/windows/texlive/tlmgr-update-done.png)

이제 최종적으로 Tex Live Shell 주화면이 표시됩니다. 모든 작업을 마쳤으므로 **Quit** 버튼을 눌러 프로그램을 마칩니다.

![TLShell 주화면](/assets/images/windows/texlive/tlshell-done.png)
